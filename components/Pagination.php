...
class Pagination
{
    ...
    public function get()
    {
        ...
        if (!is_null($links)) {
            ...
            # Создаём ссылку "На первую"
                $links = $this->generateHtml(1,'<i class="ion-ios-arrow-left"></i>') . $links;

            # Если текущая страница не последняя
            if ($this->current_page < $this->amount)
            # Создаём ссылку "На последнюю"
                $links .= $this->generateHtml($this->amount, '<i class="ion-ios-arrow-right"></i>');
        }
    ...
    private function limits()
    {
        ...
        { 
            ...
            $end = ($this->current_page == $end) ? $end + 1: $end;
